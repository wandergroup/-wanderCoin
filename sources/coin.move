module wander_address::wander {
    ...
    struct Capabilities has key {
        mint_cap: MintCapability<WanderCoin>,
        burn_cap: BurnCapability<WanderCoin>
    }

    public entry fun initialize(admin: &signer) {
        assert!(signer::address_of(admin) == @glow_address, ERR_NOT_ADMIN);
        assert!(!coin::is_coin_initialized<WanderCoin>(), ERR_COIN_INITIALIZED);

        let (burn_cap, freeze_cap, mint_cap) = coin::initialize<WanderCoin>(admin, utf8(b"WanderCoin"), utf8(b"wc"), 6, true);
    ...
    public entry fun burn(user: &signer, amount: u64) acquires Capabilities {
        ...
        let coin = coin::withdraw<WanderCoin>(user, amount);
